...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( function() {
	...
	var _DEG2RAD = Math.PI / 180,
		...
		_morphMessage = "Use TonySVGPlugin.convertToPath(elementOrSelectorText) to convert to a path before morphing.",



		TonySVGPlugin = _gsScope._gsDefine.plugin({
			propName: "tonySVG",
			API: 2,
			global: true,
			version: "0.0.1",
			...
			init: function(target, value, tween) {
				...
				if (shape) {
					...
					pt = this._addTween(target, "setAttribute", target.getAttribute(p) + "", shape + "", "tonySVG", false, p, (typeof(value.precompile) === "object") ? function(a) {a[0] = value.precompile[0]; a[1] = value.precompile[1];} : (p === "d") ? _buildPathFilter(value.shapeIndex, value.map || TonySVGPlugin.defaultMap, value.precompile) : _buildPointsFilter(value.shapeIndex));
					if (pt) {
						this._overwriteProps.push("tonySVG");
						pt.end = shape;
						pt.endProp = p;
					}
				}
	...
	TonySVGPlugin.pathFilter = _pathFilter;
	TonySVGPlugin.pointsFilter = _pointsFilter;
	TonySVGPlugin.subdivideRawBezier = _subdivideBezier;
	TonySVGPlugin.defaultMap = "size";
	TonySVGPlugin.pathDataToRawBezier = function(data) {
		return _pathDataToBezier(_parseShape(data, true));
	};
	TonySVGPlugin.equalizeSegmentQuantity = _equalizeSegmentQuantity;

	TonySVGPlugin.convertToPath = function(targets, swap) {
	...
	TonySVGPlugin.pathDataToBezier = function(data, vars) { //converts SVG path data into an array of {x, y} objects that can be plugged directly into a bezier tween. You can optionally pass in a 2D matrix like [a, b, c, d, tx, ty] containing numbers that should transform each point.
